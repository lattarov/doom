...
profiles API is stable."
  :type '(choice (const :tag "PERSONAL")
                 (const :tag "WORK"))
  :group 'vlv)
...
;; REVIEW it should avoid having pgtk build messing with
;;        the top bar, but for now this has no effect
;;        on COSMIC DE and only breaks stuff on Windows.

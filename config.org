...
(use-package! dirvish
  ...
  (setopt dirvish-quick-access-entries
          (let ((org-path (expand-file-name org-directory))
                ...
                (repos-path     (string-join (list vlv-home (if (vlv-env-work-p) "dev/@Src" "dev/")) "")))
            `(("h" ,vlv-home "Home")
...
* Workspaces
Make buffer selection follow the workspace that owns the buffer, instead of pulling the buffer into whatever workspace happens to be current. Popups and consult/vertico-style previews are excluded.
...
Must run synchronously, ahead of Doom's own `+workspaces-add-current-buffer-h'
in `doom-switch-buffer-hook' (hooks added later run first, since `add-hook'
prepends by default): that default hook unconditionally adds the current
buffer to the current workspace, so if it ran first, or if this were
deferred past it, a buffer being *selected into* another workspace would
get silently re-claimed by the workspace it's being switched away from
before this function ever got to check ownership.

Popups/special buffers are already excluded via `doom-real-buffer-p'.
Buffer previews shown while a minibuffer session is active (consult/vertico
narrowing) are ignored -- only the final, committed selection acts, since
`active-minibuffer-window' is nil by the time that happens.

`+workspace-switch' can fail to restore a workspace's saved window layout
if it no longer fits the frame (persp-mode logs a warning and leaves the
old layout in place); the buffer is still explicitly re-selected
afterwards regardless, so the visible buffer stays correct even if the
rest of that workspace's window layout didn't restore."

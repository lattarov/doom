...
(defvar vlv-environment-personal
  "PERSONAL"
  "Value set to vlv-environment if in personal environment.")

(defvar vlv-environment-work
  "WORK"
  "Value set to vlv-environment if in work environment.")

(defcustom vlv-environment vlv-environment-personal
...
"
  :type 'string
  :options '(vlv-environment-personal vlv-environment-work))

(if (eq system-type 'gnu/linux)
    (setopt vlv-environment vlv-environment-personal)
    (setopt vlv-environment vlv-environment-work))

(defun vlv-environment-work ()
  "Returns t if in work environment, nil otherwise."
  (string-equal vlv-environment vlv-environment-work))

(defun vlv-environment-personal ()
  "Returns t if in personal environment, nil otherwise."
  (string-equal vlv-environment vlv-environment-personal))

;; external doom dependencies in work environment
(when (vlv-environment-work)
  (let* ((vlv-environment-work-emacs-install-path  "c:/Emacs/")
         (vlv-environment-work-runtimes-path       (expand-file-name "runtimes/"
                                                                     vlv-environment-work-emacs-install-path))
         (vlv-environment-work-runtimes-fd-path    (expand-file-name "fd"
                                                                     vlv-environment-work-runtimes-path))
         (vlv-environment-work-runtimes-rg-path    (expand-file-name "rg"
                                                                     vlv-environment-work-runtimes-path))

         (paths-to-add (list vlv-environment-work-emacs-install-path
                             vlv-environment-work-runtimes-path
                             vlv-environment-work-runtimes-fd-path
                             vlv-environment-work-runtimes-rg-path)))
...
(use-package! doom
  :defer t
  :config
  ;; only create this function if in personal environment
  (when (vlv-environment-personal)
...
;; (setopt mu4e-get-mail-command "mbsync gmail"
...
(use-package! eww
  ...
  (defun vlv/eww ()
    ...
    (interactive)
    (setopt-local vlv-eww-workspace-name "EWW")
    (progn
...
Found in https://emacs.stackexchange.com/questions/60200/magit-remove-local-branches-that-were-merged-into-another-branch"
    ...
    (setopt magit-clone-default-directory (if (vlv-environment-personal)
                                              "~/dev/"
                                            "D:/@Src/"))

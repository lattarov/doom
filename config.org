...
Small tweaks to activate or configurate variables depending on the machine. This will ultimately be replaced by doom profiles + modules, but it should work for now.
...
(defcustom vlv-home
  (if (vlv-env-work-p)
      (if (getenv "WSL_DISTRO_NAME")
          "/mnt/c/Users/LATTARO-VOLPINI/"
        "~/")
    "~/")
  "The home directory variable, compatible between Linux, Windows and wsl systems."
  :type 'string
  :group 'vlv)
...
In order to disactivate packages, the easiest way is to use the 'disable' field of packages! macro. the vlv-env-* helper functions are quite handy for that, hence tangling them to packages.el and config.el at the same time solves all issues.
...
(when (vlv-env-personal-p)
  ...
  (setopt +dashboard-ascii-banner-fn #'vlv-doom-dashboard-draw-ascii-banner-fn))

(setopt doom-theme 'modus-vivendi-tritanopia)
...
(defvar vlv-private-config-workspace "DOOM"
  "Name of the private Doom config workspace.")

(defvar vlv-private-config-workspace "DOOM"
  "Name of the private Doom config workspace.")

(defun vlv--private-config-workspace ()
  "Switch to `vlv-private-config-workspace', creating it on first call."
  (unless (modulep! :ui workspaces)
    (user-error "The :ui workspaces module is required"))
  (unless (+workspace-exists-p vlv-private-config-workspace)
    (+workspace-new vlv-private-config-workspace)
    (projectile-add-known-project (file-truename doom-user-dir)))
  (+workspace-switch vlv-private-config-workspace))

(defun vlv-open-private-config-file (file)
  "Open FILE, relative to `doom-user-dir', in the private config workspace."
  (interactive (list (read-file-name "Private config file: " doom-user-dir)))
  (vlv--private-config-workspace)
  (prog1 (find-file (expand-file-name file doom-user-dir))
    (+workspace/display)))

(map! :leader
      :prefix ("f p" . nil)
      :prefix ("f p" . "Private config")
      :desc "Open init.el"         "i" (cmd! (vlv-open-private-config-file "init.el"))
      :desc "Open elfeed.org"      "e" (cmd! (vlv-open-private-config-file "elfeed.org"))
      :desc "Open literate config" "c" (cmd! (vlv-open-private-config-file "config.org")))
...
(when (or (vlv-env-work-p)
          (string-match "laptop" (shell-command-to-string "uname --nodename")))
...
(setopt user-full-name    "Victor Lattaro Volpini"
        user-mail-address (if (vlv-env-work-p)
                              "v.lattaro-volpini.ext@tiama.com"
                            "victorlattaro@proton.me"))
#+end_src
...
(setq recentf-autosave-interval 120)
#+end_src
...
[[https://github.com/minad/jinx][Jinx]] replaces Doom's =:checkers spell= module (commented out in =init.el=). It
binds directly to the libenchant C API through a native module instead of
talking to an =aspell= subprocess, checks only the *visible* portion of the
buffer, and picks regions by face - so =global-jinx-mode= covers both
=flyspell-mode= and =flyspell-prog-mode=, restricting itself to comments and
strings in code.

Crucially it loads several dictionaries at once and accepts a word if *any* of
them knows it, which is what makes brazilian portuguese, english and french
work simultaneously in the same buffer.

System prerequisites (Debian/Ubuntu):

#+begin_src sh :tangle no
sudo apt install libenchant-2-dev pkgconf hunspell-pt-br hunspell-fr aspell-en
#+end_src

Caveat: the accepted vocabulary is the *union* of the three languages, so
homographs (=pas=, =son=, =mais=, =on=, =mode=, =cor=) let some real typos
through. Narrow the set per buffer with =C-M-$=, per project with a
=.dir-locals.el= entry for =jinx-languages=, or per file with =#+language:=.
...
#+begin_src emacs-lisp :tangle config.el
;; The native module needs libenchant headers + a C compiler, which we do
;; not have on native Windows. WSL is fine.
(unless (eq system-type 'windows-nt)
  ...
  (after! jinx
    ;; Checked in parallel: a word is valid if ANY of these accepts it.
    (setopt jinx-languages "en_US pt_BR fr_FR")

    ;; Doom already turns on `vertico-multiform-mode'; a grid fits suggestions
    ;; better than the default vertical list.
...
** Basic config
#+begin_src emacs-lisp :tangle config.el
(setopt org-directory (string-join
                       (list vlv-home (if (vlv-env-work-p)
                                          "OneDrive - Kestrel Vision/"
                                        "")
                             "org/") ""))
...
(after! org-journal
  ...
  ;; integrate org-journal with doom's pop up mechanism
  (defun +org-journal-find-file-popup (path)
    "Open PATH in a Doom popup. For `org-journal-find-file'."
    (pop-to-buffer (find-file-noselect path)))

  (set-popup-rule!
    (lambda (bufname _action)
      (and (bound-and-true-p org-journal-dir)
           (when-let* ((buf  (get-buffer bufname))
                       (file (buffer-file-name buf)))
             (file-in-directory-p file org-journal-dir))))
    :side     'bottom
    :size     0.4
    :select   t
    :modeline t
    :quit     nil    ; ESC won't nuke a half-written entry
    :ttl      nil)   ; keep the file buffer alive after closing)

  (defadvice! +org-journal-popup-a (fn &rest args)
    :around #'org-journal-new-entry
    (let ((org-journal-find-file-fn #'+org-journal-find-file-popup))
      (apply fn args))))

(map! :after org-journal
      :map org-journal-mode-map
      :localleader
      "z" (cmd! (save-buffer) (+popup/close nil t)))
#+end_src
...
(after! dired
  (setopt dired-async-mode       t
          dired-listing-switches "-alh --no-group -g --sort=extension"))
#+end_src
...
(after! dirvish
  (setopt dirvish-quick-access-entries
          (let ((org-path (expand-file-name org-directory))
                (documents-path (string-join (list vlv-home "One Drive - Kestrel Visio/Documents/") ""))
                (downloads-path (string-join (list vlv-home "Downloads/") "")))
            `(("h" "~" "Home")
              ("d" ,downloads-path "Downloads")
              ("D" ,documents-path "Documents")
              ("o" ,org-path "Org")
              ("r" "~/dev/" "Repos")
              ("c" "/mnt/c/" "C:/")
              ("d" "/mnt/d/" "D:/"))))
...
** Claude Code
[[https://github.com/manzaltu/claude-code-ide.el][claude-code-ide.el]] runs the =claude= CLI inside Emacs, so it uses the Claude Code
subscription (log in once with =claude= → =/login=).  gptel can't: it calls the
API directly, and subscription logins are not allowed in third-party clients.
...
Terminals come from Doom's =:term= modules (=SPC o t= / =SPC o T=), and Claude
Code reuses whichever one is enabled.  Its keys sit under the =:tools llm=
prefix (=SPC o l=), next to gptel's.

#+begin_src emacs-lisp :tangle config.el
(defcustom vlv-claude-code-width-ratio 0.4
  "Share of the frame width taken by the Claude Code side window."
  :type 'number
  :group 'vlv)

(defcustom vlv-claude-code-min-width 100
  "Minimum width, in columns, of the Claude Code side window."
  :type 'integer
  :group 'vlv)

(defun vlv/claude-code ()
  "Toggle the project's Claude Code window, starting Claude if none runs."
  (interactive)
  (setopt claude-code-ide-window-width
          (max vlv-claude-code-min-width
               (floor (* vlv-claude-code-width-ratio (frame-width)))))
  (condition-case nil
      (claude-code-ide-toggle)
    (user-error (claude-code-ide))))

(after! claude-code-ide
  (setopt claude-code-ide-terminal-backend
          (cond ((modulep! :term ghostel) 'ghostel)
                ((modulep! :term vterm) 'vterm)
                (t 'eat)))
  (claude-code-ide-emacs-tools-setup))

(map! :leader
      :prefix "o l"
      :desc "Toggle Claude Code"         "c" #'vlv/claude-code
      :desc "Claude Code menu"           "C" #'claude-code-ide-menu
      :desc "Send prompt to Claude"      "p" #'claude-code-ide-send-prompt
      :desc "Insert selection in Claude" "i" #'claude-code-ide-insert-at-mentioned)
#+end_src
...
(after! consult-mu
  ...
  (setopt consult-mu-preview-key 'any)

  (setopt consult-mu-mark-previewed-as-read nil)

  (setopt consult-mu-mark-viewed-as-read nil)

  (setopt consult-mu-action #'consult-mu--view-action))
#+end_src
...
(setopt consult-omni-show-preview t) ;;; show previews
;; consult-omni's :type spec for this defcustom uses invalid widget
;; names, so `setopt' crashes here validating against it (upstream bug);
;; `setq' skips that validation.
(setq consult-omni-preview-key "C-o") ;;; set the preview key to C-o
...
(when (vlv-env-personal-p)
  ...
  (set-email-account! "@gmail.com"
                      '((mu4e-sent-folder       . "/gmail/Sent Mail")
                        ...
                        (mu4e-refile-folder     . "/gmail/All Mail") (smtpmail-smtp-user     . "victorlattaro@gmail.com")
                        (message-signature . "Victor Lattaro Volpini")))
  ...
  (setq message-send-mail-function 'smtpmail-send-it
        ...
        '(("smtp.gmail.com" 587 "victorlattaro@gmail.com" nil))
...
(defcustom vlv-vncviewer-path (string-join (list (if (getenv "WSL_DISTRO_NAME")
                                                     "/mnt/c/"
                                                   "C:/")
                                                 "Program Files/TightVNC/tvnviewer.exe") "")
...
(defcustom vlv--workbench3-path
  "/mnt/c/WindRiver_6947/workbench-3.3/wrwb/platform/x86-win32/eclipse/wrwb-x86-win32.exe"
  "The path to the executable or script to start WindRiver 3 workbench IDE."
  :type 'file
  :group 'work)

(defcustom vlv--workbench4-path
  "/mnt/c/089-VXWORKS_CALIA4/workbench-4/startWorkbench.bat"
...
#+begin_src emacs-lisp :tangle config.el
(after! consult-everything
  ;; the .exe makes it work on wsl
  (setopt consult-everything-args "es.exe -r"))
...
#+begin_src emacs-lisp :tangle config.el
(defun vlv-env-work-gen-payfit-report (ON-SITE-DAYS HOME-OFFICE-DAYS &optional COMMENT?)
  "Create the monthly comment message for payfit.

ON-SITE-DAYS and HOME-OFFICE-DAYS are self descriptive.
COMMENT is an optional string appended to a new line."
  (interactive "nOn site days: \nnHome Office days:")
  (let* ((on-site-fee (* ON-SITE-DAYS 15))
         (home-office-fee (* HOME-OFFICE-DAYS 10))
         (total (+ on-site-fee home-office-fee))
         (comment (if COMMENT? (format " \n %s" COMMENT?) ""))
         (text (format "%d jours x 15€/jour sur site + %d jours x 10€/jour TT = %d€ + %d€ = %d%s€"
                       ON-SITE-DAYS HOME-OFFICE-DAYS on-site-fee home-office-fee total comment)))
    
                                        ; send the text to clipboard and log
    (kill-new text)
...
#+begin_src emacs-lisp :tangle packages.el
(package! spotify
  :disable (vlv-env-work-p)
  :pin "d918b5187638e0c44a2a2584f3980244b6aae3fa")
#+end_src

#+begin_src emacs-lisp :tangle config.el
;; WARNING deferring load (e.g. via `after!') makes the package not be loaded when using bindings.
(require 'spotify)
...
#+begin_src emacs-lisp :tangle config.el
(after! persp-mode
  (defun vlv-workspace-follow-buffer-h ()
    "Switch to the workspace that owns the just-selected buffer."
    (when (and (not (active-minibuffer-window))
               (doom-real-buffer-p (current-buffer)))
      (let ((buf (current-buffer)))
        (unless (+workspace-contains-buffer-p buf (+workspace-current))
          (when-let* ((owner (cl-find-if
                              (lambda (wk) (+workspace-contains-buffer-p buf wk))
                              (+workspace-list))))
            (condition-case err
                (+workspace-switch (persp-name owner))
              (error (message "vlv-workspace-follow-buffer-h: switch to %s failed: %S"
                              (persp-name owner) err)))
            (when (buffer-live-p buf)
              (switch-to-buffer buf)))))))

  (add-hook 'doom-switch-buffer-hook 'vlv-workspace-follow-buffer-h))
#+end_src
...
(after! ibuffer
  ...
  (when (modulep! :ui workspaces)
    ...
    (defun vlv-ibuffer-set-workspace-filter-groups-h ()
      (setq ibuffer-filter-groups
            (mapcar (lambda (name)
                      (cons name (list (cons 'workspace-buffers (+workspace-get name)))))
                    (+workspace-list-names))))
    (add-hook 'ibuffer-hook 'vlv-ibuffer-set-workspace-filter-groups-h)))
#+end_src
...
(quiet! (load-file (expand-file-name "conventional-commits-snippets.el"
                                     doom-user-dir)))
#+end_src
...
* evil-mode
#+begin_src emacs-lisp :tangle config.el
;; evil is loaded before config.el, hence wrapping this in an `after!' block
;; makes this block never to be run.

(setopt evil-shift-width 30)
#+end_src
...
#+begin_src emacs-lisp :tangle config.el
(after! mastodon
  (setopt mastodon-active-user "victorlattaro")
...
(after! alarm-clock
  (when (vlv-env-work-p)
    (alarm-clock-set "08:30" "🍵 Coffee time!")
    (alarm-clock-set "10:00" "🍵 Coffee time!")
    (alarm-clock-set "12:00" "nham nham!")
    (alarm-clock-set "15:00" "🍵 Coffee time!")
    (alarm-clock-set "17:00" "Go home!")))
#+end_src
...
#+begin_src emacs-lisp :tangle config.el
(map! :leader "o j" #'journalctl
      :after journalctl
      :map journalctl-mode-map
      :nv "q" #'journalctl-quit)
#+end_src

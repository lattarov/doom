...
(after! gptel
  ...
  (setopt gptel-backend (gptel-make-ollama "Ollama"
          ...
          :stream t
          :models '((deepseek-coder-v2:16b) (ifioravanti/llamantino-2) (deepseek-r1:8b)))))
#+end_src
...
(defun vlv/open-windriver-workbench ()
  "Open Wind River Workbench IDE."
    (interactive)
...
** Tree-sitter
*** treesit-auto
Sometimes (especially on windows) the ts-* mode is always on even if the tree-sitter flag is off in init.el. Treesit-auto allows to detect if the grammar of the target language is installed prior to activating the <language>-mode or ts-<language>-mode, only activating the latter if the grammar is present.

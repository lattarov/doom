...
(defun vlv/org-agenda-open-hook()
"Hook to be called when opening an org agenda buffer"
(olivetti-mode)
(olivetti-set-width 120))

(add-hook 'org-agenda-mode-hook  'vlv/org-agenda-open-hook)

(setq org-agenda-files '("~/org/agenda/" "~/org/roam/" "~/org/" "~/org/journal/" "~/.config/doom/"))
#+end_src
...
(use-package! consult-omni
  ...
  (setq consult-omni-sources-modules-to-load (list 'consult-omni-duckduckgo-api 'consult-omni-notes 'consult-omni-wikipedia  'consult-omni-elfeed  'consult-omni-file 'consult-omni-buffer 'consult-omni-buffer-search 'consult-omni-dict))
...
***** TODO add guide on how to create an application-specific password on google
***** TODO setup smtp server or 2FA for gmail
***** TODO configure for proton mail
****** TODO install proton bridge
**** Mu4e
...
*** TODO Configuration (fix error with mu4e)
...
Mas recommended in doom's mu4e docs as well as in [[https://github.com/doomemacs/doomemacs/issues/7885][doomemacs/doomemacs#7885 Can't open mu4e]] , manually adding the load path fixes this.
...
(setq +mu4e-gmail-accounts '(("victorlattaro@gmail.com" . "/gmail")))
